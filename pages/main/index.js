...
export class MainPage {
    ...
    handleAddProduct() {
        ...
        const newProduct = {
            ...
            name: `Копия: ${this.products[0].name}`
        };
        console.log(newProduct)

        ajax.post(productUrls.createProduct(), newProduct, (data, status) => {
            if (status >= 200 && status < 300) {
                this.products.push(data);
                this.renderProducts(this.products);
            } else {
                console.error('Ошибка при добавлении:', status);
            }
        });
    }

    handleDeleteProduct(id) {
        ajax.delete(productUrls.removeProductById(id), (data, status) => {
            if (status >= 200 && status < 300) {
